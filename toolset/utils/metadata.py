...
class Metadata:
    ...
    def gather_tests(self, include=None, exclude=None):
        ...
        if self.benchmarker.config.test_dir:
            ...
            config_files.extend(
                glob.glob(
                    "{!s}/*/*/benchmark_config.json".format(
        ...
        for config_file_name in config_files:
            ...
            with open(config_file_name, "r") as config_file:
                try:
                    ...
                    log(
                        "Error loading config: {!s}".format(config_file_name),
                        color=Fore.RED,
                    )
    ...
    def tests_to_run(self):
        ...
        return self.gather_tests(
            self.benchmarker.config.test, self.benchmarker.config.exclude
        )
    ...
    def parse_config(self, config, directory):
        ...
        for test in config["tests"]:
            ...
            for test_name, test_keys in test.iteritems():
                ...
                test_keys = Metadata.validate_test(
                    test_name, test_keys, config["framework"], directory
                )
    ...
    def list_test_metadata(self):
        ...
        with open(
            os.path.join(self.benchmarker.results.directory, "test_metadata.json"), "w"
        ) as f:
    ...
    def validate_test(test_name, test_keys, project_name, directory):
        ...
        schema = {
            ...
            "database": {
                "allowed": Metadata.supported_dbs
                + [
                    (
                        "None",
            ...
            "approach": {"allowed": [("Realistic", "..."), ("Stripped", "...")]},
            "orm": {
