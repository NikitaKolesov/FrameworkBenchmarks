...
mem_limit = int(round(virtual_memory().total * 0.95))
...
class DockerHelper:
    ...
    def __build(
        self, base_url, path, build_log_file, log_prefix, dockerfile, tag, buildargs={}
    ):
        """
            ...
            try:
                ...
                for token in output:
                    ...
                    if self.benchmarker.time_logger.time_since_start() > 3600:
                        log(
                            "Build time exceeded 60 minutes",
                            prefix=log_prefix,
                            file=build_log,
                            color=Fore.RED,
                        )
                ...
                if buffer:
                    log(
                        ...
                        file=build_log,
                        color=Fore.WHITE + Style.BRIGHT
                        if re.match(r"^Step \d+\/\d+", buffer)
                        else "",
                    )
            except Exception:
                tb = traceback.format_exc()
                log(
                    "Docker build failed; terminating",
                    prefix=log_prefix,
                    file=build_log,
                    color=Fore.RED,
                )
    ...
    def run(self, test, run_log_dir):
        ...
        try:
            ...
            ulimit = [
                {"name": "nofile", "hard": 200000, "soft": 200000},
            ...
            watch_thread = Thread(
                target=watch_container, args=(container, "%s.dockerfile" % test.name)
            )
            ...
            with open(
                os.path.join(run_log_dir, "%s.log" % test.name.lower()), "w"
            ) as run_log:
                tb = traceback.format_exc()
                log(
                    "Running docker container: %s.dockerfile failed" % test.name,
                    prefix=log_prefix,
                    file=run_log,
                )
    ...
    def stop(self, containers=None):
        ...
        """
        is_multi_setup = (
            self.benchmarker.config.server_docker_host
