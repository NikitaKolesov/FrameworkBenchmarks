...
class TimeLogger:
    ...
    def log_database_start_time(self, log_prefix, file):
        log(
            "Time starting database: %s" % TimeLogger.output(self.database_started),
            prefix=log_prefix,
            file=file,
            color=Fore.YELLOW,
        )
    ...
    def log_benchmarking_end(self, log_prefix, file):
        ...
        log(
            "Benchmarking time: %s" % TimeLogger.output(total),
            prefix=log_prefix,
            file=file,
            color=Fore.YELLOW,
        )
    ...
    def log_test_accepting_requests(self, log_prefix, file):
        log(
            "Time until accepting requests: %s"
    ...
    def log_test_end(self, log_prefix, file):
        ...
        log(
            "Total test time: %s" % TimeLogger.output(total),
            prefix=log_prefix,
            file=file,
            color=Fore.YELLOW,
        )
        log(
            "Total time building so far: %s" % TimeLogger.output(self.build_total),
            prefix="tfb: ",
            file=file,
            color=Fore.YELLOW,
        )
        log(
            "Total time verifying so far: %s" % TimeLogger.output(self.verify_total),
            prefix="tfb: ",
            file=file,
            color=Fore.YELLOW,
        )
        if self.benchmarking_total > 0:
            log(
                "Total time benchmarking so far: %s"
        ...
        log(
            "Total execution time so far: %s" % TimeLogger.output(running_time),
            prefix="tfb: ",
            file=file,
            color=Fore.YELLOW,
        )
    ...
    def log_verify_end(self, log_prefix, file):
        ...
        log(
            "Verify time: %s" % TimeLogger.output(total),
            prefix=log_prefix,
            file=file,
            color=Fore.YELLOW,
        )
