...
class Results:
    def __init__(self, benchmarker):
        ...
        self.directory = os.path.join(self.config.results_root, self.config.timestamp)
        try:
    ...
    def parse_test(self, framework_test, test_type):
        ...
        with open(
            self.get_stats_file(framework_test.name, test_type) + ".json", "w"
        ) as stats_file:
    ...
    def finish(self):
        ...
        if not self.config.parse:
            ...
            log("Verification Summary", border="=", border_bottom="-", color=Fore.CYAN)
            for test in self.benchmarker.tests:
                log(Fore.CYAN + "| {!s}".format(test.name))
                if test.name in self.verify.keys():
                    for test_type, result in self.verify[test.name].iteritems():
    ...
    def __count_sloc(self):
        ...
        for framework, testlist in frameworks.items():
            ...
            # one file listed.
            command = (
                "cloc --yaml --follow-links . | grep code | tail -1 | cut -d: -f 2"
            )
            ...
            try:
                ...
                log(
                    "Unable to count lines of code for %s due to error '%s'"
                    % (framework, e)
                )
                continue
    ...
    def __get_git_commit_id(self):
        ...
        return subprocess.check_output(
            ["git", "rev-parse", "HEAD"], cwd=self.config.fw_root
        ).strip()
    ...
    def __get_git_repository_url(self):
        ...
        """
        return subprocess.check_output(
            ["git", "config", "--get", "remote.origin.url"], cwd=self.config.fw_root
        ).strip()
    ...
    def __get_git_branch_name(self):
        ...
        """
        return subprocess.check_output(
            "git rev-parse --abbrev-ref HEAD", shell=True, cwd=self.config.fw_root
        ).strip()
