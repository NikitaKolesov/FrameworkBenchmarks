...
def fw_found_in_changes(test, changes_output):
    return re.search(r"frameworks/" + re.escape(test) + "/", changes_output, re.M)
...
changes = clean_output(
    subprocess.check_output(
        [
            "bash",
            "-c",
            "git --no-pager diff --name-only {0} $(git merge-base {0} master)".format(
                curr_branch
            ),
        ]
    )
...
print(
    "Determining what to run based on the following file changes: \n{!s}".format(
        "\n".join(changes.split("\n")[0:10])
    )
)
...
if re.search(r"\[ci fw-only .+\]", last_commit_msg, re.M):
    tests = (
        re.findall(r"\[ci fw-only (.+)\]", last_commit_msg, re.M)[0].strip().split(" ")
    )
    for test in tests:
...
if re.search(r"\[ci lang-only .+\]", last_commit_msg, re.M):
    langs = (
        re.findall(r"\[ci lang-only (.+)\]", last_commit_msg, re.M)[0]
        .strip()
        .split(" ")
    )
    for test in test_dirs:
        for lang in langs:
            if test.startswith(lang + "/"):
                print(
                    "{!s} has been forced to run from the commit message.".format(test)
                )
                run_tests.append(test)
...
if (
    re.search(r"^toolset\/(?!(travis\/|continuous\/|scaffolding\/))", changes, re.M)
    is not None
):
