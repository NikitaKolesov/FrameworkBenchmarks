...
class FrameworkTest:
    ...
    def start(self):
        ...
        test_log_dir = os.path.join(
            self.benchmarker.results.directory, self.name.lower()
        )
    ...
    def is_accepting_requests(self):
        ...
        url = "http://%s:%s%s" % (
            self.benchmarker.config.server_host,
            self.port,
    ...
    def verify_urls(self):
        ...
        def verify_type(test_type):
            ...
            try:
                ...
                pass
            with open(
                os.path.join(verificationPath, "verification.txt"), "w"
            ) as verification:
                test = self.runTests[test_type]
                log(
                    "VERIFYING %s" % test_type.upper(),
                    file=verification,
                    border="-",
            ...
            including this message: %s\n%s"""
                ...
                def output_result(result, reason, url):
                    ...
                    if reason is not None and len(reason) != 0:
                        ...
                        if not test.passed:
                            log(
                                "     See {!s}".format(specific_rules_url),
                                file=verification,
                            )
                ...
                if test.failed:
                    ...
                    self.benchmarker.results.report_verify_results(
                        self, test_type, "fail"
                    )
                elif test.warned:
                    test.output_headers_and_body()
