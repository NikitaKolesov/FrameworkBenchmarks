...
def verify_headers(request_headers_and_body, headers, url, should_be="json"):
    ...
    for v in (
        v for v in ("Server", "Date", "Content-Type") if v.lower() not in headers
    ):
        problems.append(("fail", "Required response header missing: %s" % v, url))
    ...
    if "message" not in json_object:
        ...
        if json_len > 1:
            ...
            problems.append(
                (
                    "warn",
                    "Too many JSON key/value pairs, consider removing: %s" % additional,
                    url,
                )
            )
        if json_len > 27:
        ...
        if message != "hello, world!":
            return [
                ("fail", "Expected message of 'hello, world!', got '%s'" % message, url)
            ]
        return problems
...
def verify_randomnumber_object(db_object, url, max_infraction="fail"):
    ...
    if len(db_object) > len(required_keys):
        ...
        problems.append(
            (
                "warn",
                "An extra key(s) is being included with the db object: %s"
                % ", ".join(extras),
                url,
            )
        )
    ...
    try:
        ...
        problems.append(
            (
                max_infraction,
                "Response key 'id' does not map to an integer - %s" % e,
                url,
            )
        )
...
def verify_randomnumber_list(expected_len, headers, body, url, max_infraction="fail"):
    ...
    if type(response) is not list:
        problems.append(
            (max_infraction, "Top-level JSON is an object, not an array", url)
        )
...
def verify_updates(old_worlds, new_worlds, updates_expected, url):
    ...
    if successful_updates == 0:
        problems.append(("fail", "No items were updated in the database.", url))
    ...
    for q, max_infraction in cases:
        ...
        try:
            ...
            if check_updates and queries >= MAX:
                ...
                problems += verify_updates(
                    world_db_before, world_db_after, MAX, case_url
                )
            ...
            if body is None:
                problems.append(
                    (max_infraction, warning % ("No response", q), case_url)
                )
            elif len(body) == 0:
                problems.append(
                    (max_infraction, warning % ("Empty response", q), case_url)
                )
            else:
                expected_len = 1
                ...
                problems += verify_headers(
                    self.request_headers_and_body, headers, case_url
                )
