...
async def single_db_query_test(request):
    ...
    try:
        number = await connection.fetchval(
            'SELECT "randomnumber" FROM "world" WHERE id = $1', row_id
        )
    ...
    return Response(
        200,
        content=Content(
            b"application/json; charset=utf-8", json_dumps(world).encode("utf-8")
        ),
    )
...
async def fortunes_test(request):
    ...
    return Response(
        ...
        content=Content(
            ...
            fortune_template.render(fortunes=fortunes).encode("utf8"),
        ),
    )
...
async def db_updates_test(request):
    ...
    return Response(
        200, content=Content(b"application/json", json_dumps(worlds).encode("utf-8"))
    )
