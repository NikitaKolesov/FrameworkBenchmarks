...
WRITE_ROW_SQL = (
    'UPDATE "world" SET "randomnumber"={0} WHERE id={1} RETURNING id, randomNumber'
)
...
con = psycopg2.connect(
    "dbname=hello_world user=benchmarkdbuser host=tfb-database password=benchmarkdbpass"
)
cur = con.cursor()
...
async def single_query(request: Request):
    res = await fetchWorld()
    return JsonResponse(
        {"id": res[0], "randomNumber": res[1]}, headers={"Server": "Vibora"}
    )
...
async def plaintext():
    return Response(
        b"Hello, World!", headers={"Server": "Vibora", "Content-Type": "text/plain"}
    )
