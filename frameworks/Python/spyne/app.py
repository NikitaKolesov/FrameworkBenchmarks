...
class TfbRawService(ServiceBase):
    ...
    def updatesraw(ctx, queries):
        ...
        for i in ids:
            ...
            conn.execute("UPDATE World SET randomNumber=%s WHERE id=%s", rn, wid)
...
def init(config):
    return {
        "root": ServiceDefinition(
            init=init_app, default=HttpServer(type="tcp4", host="127.0.0.1", port=8080)
        )
    }
    ...
    def after_tables(self, config):
        ...
        with closing(config.get_main_store().Session()) as session:
            ...
            for _ in range(100):
                session.add(
                    Fortune(
                        message=" ".join([choice(words) for _ in range(randint(3, 10))])
                    )
                )
