...
raw_engine = create_engine(
    DATABASE_URI, connect_args={"autocommit": True}, pool_reset_on_return=None
)
...
def get_random_world_single_raw():
    ...
    try:
        result = connection.execute(
            "SELECT id, randomNumber FROM world WHERE id = " + str(wid)
        ).fetchone()
...
def get_random_world_raw():
    ...
    try:
        for i in xrange(num_queries):
            result = connection.execute(
                "SELECT id, randomNumber FROM world WHERE id = " + str(rp())
            ).fetchone()
...
def fortune_raw():
    ...
    try:
        fortunes = [
            (f.id, f.message) for f in connection.execute("SELECT * FROM Fortune")
        ]
...
def raw_updates():
    ...
    for i in xrange(num_queries):
        ...
        conn.execute(
            "UPDATE World SET randomNumber=%s WHERE id=%s", (randomNumber, world["id"])
        )
    conn.close()
    response.content_type = "application/json"
    return json.dumps(worlds)
