...
class Container(api_hour.Container):
    def __init__(self, *args, **kwargs):
        ...
        self.servers["http"].router.add_route(
            "GET", "/db_mysql", endpoints.world.db_mysql
        )
        self.servers["http"].router.add_route(
            "GET", "/queries", endpoints.world.queries
        )
        self.servers["http"].router.add_route(
            "GET", "/queries_mysql", endpoints.world.queries_mysql
        )
        self.servers["http"].router.add_route(
            "GET", "/fortunes", endpoints.world.fortunes
        )
        self.servers["http"].router.add_route(
            "GET", "/fortunes_mysql", endpoints.world.fortunes_mysql
        )
        self.servers["http"].router.add_route(
            "GET", "/updates", endpoints.world.updates
        )
        self.servers["http"].router.add_route(
            "GET", "/updates_mysql", endpoints.world.updates_mysql
        )
        self.servers["http"].router.add_route(
            "GET", "/plaintext", endpoints.world.plaintext
        )

    def make_servers(self):
        return [
            self.servers["http"].make_handler(
    ...
    def start(self):
        ...
        self.engines["pg"] = self.loop.create_task(
            aiopg.create_pool(
                host="tfb-database",
        ...
        self.engines["mysql"] = self.loop.create_task(
